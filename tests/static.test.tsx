...
test("static output contains one heading and approved public content before JavaScript", () => {
  ...
  assert.ok(!html.includes("CV.pdf"), "Do not ship the obsolete Resume");
});
...
test("career and certifications include approved history without invented verification links", () => {
  const html = renderPage();
  for (const text of [
    "Oracle NetSuite Technical Consultant",
    "Hand Global Solutions",
    "VB.NET Developer",
    "Indomaret Group",
    "Software Engineer Intern",
    "PT Mattel Indonesia",
    "11 internal applications",
    "AWS Academy Cloud Foundations",
  ]) {
    assert.ok(html.includes(text), text);
  }
  assert.match(html, /datetime="2022-11"/i);
  const certifications = html.slice(
    html.indexOf('id="certifications"'),
    html.indexOf('id="writing"'),
  );
...
test("about and contact show supplied education, languages, identity, and safe contact fallback", () => {
  ...
  assert.ok(!html.includes("A current Resume will be added"));
});

test("confirmed LinkedIn appears in Contact and Footer", () => {
  const html = renderPage();
  assert.equal(
    (html.match(/href="https:\/\/www.linkedin.com\/in\/abrahamnaiborhu"/g) ?? []).length,
    2,
  );
});
