...
test('mobile drawer manages focus, Escape, scroll lock, navigation and resize', async ({ page }) => {
  ...
  await expect(drawer.getByRole('link', { name: 'Résumé' })).toBeFocused();
...
test('mobile navigation works without JavaScript', async ({ browser }) => {
  ...
  await page.locator('.mobile-navigation').getByRole('link', { name: 'Writing', exact: true }).click();
  await expect(page).toHaveURL(/#writing$/);
  await context.close();
});

test('signature slice has a conceptual diagram, four credentials, and active anchors', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.goto('/#writing');
  await expect(page.locator('.desktop-navigation a[href="#writing"]')).toHaveAttribute('aria-current', 'location');
  ...
  await expect(page.locator('#hero-title')).toHaveText('I build reliable cloud infrastructure and delivery systems.');
});

test('topology runs once, rests, and resets when reduced motion changes', async ({ page }) => {
  ...
  for (const path of await page.locator('.flow-emphasis').all()) await expect(path).toHaveCSS('opacity', '0');
  await page.emulateMedia({ reducedMotion: 'reduce' });
...
test('mobile metadata and scaled SVG labels respect the 14px minimum', async ({ page }) => {
  ...
  await page.goto('/');
  for (const label of await page.locator('.topology figcaption, .credential-strip span, .hero-foot a').all()) {
