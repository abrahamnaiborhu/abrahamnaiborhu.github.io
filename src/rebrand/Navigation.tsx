...
export function Navigation() {
  ...
  React.useEffect(() => {
    ...
    const resize = () => { if (desktop.matches) { modal?.close(); if (details.current) details.current.open = false; } };
    desktop.addEventListener('change', resize);
    return () => { desktop.removeEventListener('change', resize); modal?.close(); unlock(); };
  }, []);
  ...
  const onClose = () => {
    ...
    if (destination.current) {
      ...
      destination.current = null;
    } else if (window.matchMedia('(max-width: 767px)').matches) trigger.current?.focus({ preventScroll: true });
    else document.querySelector<HTMLAnchorElement>('.desktop-navigation a')?.focus({ preventScroll: true });
  };
  ...
  const links = (inDialog = false) => navigationItems.map(([id, label]) => (
    <a key={id} href={`#${id}`} aria-current={active === id ? 'location' : undefined}
      onClick={inDialog ? event => navigate(event, id) : undefined}>{label}</a>
  ));

  return <header className="navigation-shell" data-scrolled={scrolled}>
